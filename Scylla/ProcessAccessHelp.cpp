...
bool ProcessAccessHelp::openProcessHandle(DWORD dwPID)
{
    ...
    {
        // Sentinel handle: the real process access happens in x64dbg through ScyllaXBridge.
        hProcess = (HANDLE)(DWORD_PTR)0x53435844;
        return true;
    }
...
bool ProcessAccessHelp::readMemoryPartlyFromProcess(DWORD_PTR address, SIZE_T size, LPVOID dataBuffer)
{
    if (XDbgBridge::IsEnabled())
    {
        ...
        BYTE* out = (BYTE*)dataBuffer;
        SIZE_T done = 0;
        while (done < size)
        {
            SIZE_T chunk = ((size - done) > 0x1000) ? 0x1000 : (size - done);
            if (!readMemoryFromProcess(address + done, chunk, out + done))
                ZeroMemory(out + done, chunk);
            done += chunk;
        }
        return true;
    }
...
{
    if (XDbgBridge::IsEnabled())
        return XDbgBridge::WriteMemory((unsigned long long)address, dataBuffer, size);
...
{
    if (XDbgBridge::IsEnabled())
        return XDbgBridge::ReadMemory((unsigned long long)address, dataBuffer, size);
...
bool ProcessAccessHelp::getProcessModules(HANDLE hProcess, std::vector<ModuleInfo> &moduleList)
{
    ...
    {
        moduleList.clear();
        std::vector<XDbgModuleInfoWire> xmods;
        if (!XDbgBridge::GetModules(xmods))
            return false;

        moduleList.reserve(xmods.size());
        for (size_t i = 0; i < xmods.size(); i++)
        {
            ModuleInfo module;
            module.modBaseAddr = (DWORD_PTR)xmods[i].base;
            module.modBaseSize = (DWORD)xmods[i].size;
...
bool ProcessAccessHelp::getMemoryRegionFromAddress(DWORD_PTR address, DWORD_PTR * memoryRegionBase, SIZE_T * memoryRegionSize)
{
    if (XDbgBridge::IsEnabled())
    {
        ...
        if (!XDbgBridge::QueryMemory((unsigned long long)address, region))
            return false;
        *memoryRegionBase = (DWORD_PTR)region.base;
        *memoryRegionSize = (SIZE_T)region.size;
        return true;
    }
...
SIZE_T ProcessAccessHelp::getSizeOfImageProcess(HANDLE processHandle, DWORD_PTR moduleBase)
{
    if (XDbgBridge::IsEnabled())
    {
        unsigned long long size = XDbgBridge::GetModuleSizeByBase((unsigned long long)moduleBase);
        return (SIZE_T)size;
    }
