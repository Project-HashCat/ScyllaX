...
std::vector<Process>& ProcessLister::getProcessListSnapshotNative()
{
    ...
    if (XDbgBridge::IsEnabled())
    {
        ...
        if (XDbgBridge::GetProcessInfo(info))
        {
            Process process;
            process.PID = info.pid ? info.pid : 0xDB6DB6;
            process.sessionId = info.sessionId;
            process.imageBase = (DWORD_PTR)info.imageBase;
            process.imageSize = (DWORD)info.imageSize;
            process.currentIp = (DWORD_PTR)info.currentIp;
            if (info.entryPointVa && info.entryPointVa >= info.imageBase)
                process.entryPoint = (DWORD)(info.entryPointVa - info.imageBase);
            else
                process.entryPoint = 0;
            wcscpy_s(process.filename, info.filename[0] ? info.filename : L"xdbg-debuggee.exe");
            wcscpy_s(process.fullPath, info.fullPath[0] ? info.fullPath : process.filename);
